...
import { Card, Button, Grid, Box, Stack } from "@mui/material";
...
export default function ProjectCards({ data }) {
    ...
    var printtag = tag.map((e, i) => { return (<p key={i} style={{ paddingRight: 10, height: 24, margin: 0 }}>{e}</p>) })
        ...
        <Grid item md={6} lg={6} sm={12}>
            <Card sx={{ background: "none" }} elevation={0}>
                <div className="Image" onMouseEnter={() => setHover(true)}
                    ...
                    {isHovered &&
                        <Box className="overlay" style={{ zIndex: 1, width: "645px", height: "477px", position: "absolute", display: "flex", alignItems: "center", color: "white", background: "rgb(36 36 36 / 50%)", justifyContent: "center" }}>
                            <Stack direction="column" spacing={1}>
                                <Button sx={{ color: "white", padding: 0, marginRight: 10, width: 120, borderBottom: 5, borderRadius: 0, borderColor: "#4EE1A0" }}>View Project</Button>
                                <Button sx={{ color: "white", padding: 0, marginRight: 10, width: 120, borderBottom: 5, borderRadius: 0, borderColor: "#4EE1A0" }}>View Code</Button>
                            </Stack>
                        </Box>}
                    <img src={images[img]} style={{ height: "100%", width: "100%", position: "relative" }} />
                </div>
